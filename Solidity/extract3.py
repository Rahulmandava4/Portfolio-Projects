#!/usr/bin/env python3
"""
extract_repo_smart_contracts.py

Fetch the full tag, release, and commit history (all time) for the
`carl-egge/repo-smart-contracts` GitHub repository, then validate the
fetched data against JSON schemas, and save results to JSON and CSV:

- JSON:  repo-smart-contracts_version_history.json
- CSV:   repo-smart-contracts_commits.csv

Usage:
    python extract_repo_smart_contracts.py
...
def fetch_tags():
    ...
    while True:
        resp = requests.get(
            f"{BASE_URL}/tags", headers=HEADERS,
...
def fetch_releases():
    ...
    while True:
        resp = requests.get(
            f"{BASE_URL}/releases", headers=HEADERS,
            params={"per_page": 100, "page": page}
        )
        resp.raise_for_status()
        batch = resp.json()
