#!/usr/bin/env python3
"""
extract_stablecoin_evm.py

Fetch the full tag, release, and commit history (all time) for the
`circlefin/stablecoin-evm` GitHub repository, then validate the
fetched data against JSON schemas, and save results to JSON and CSV:

- JSON:  stablecoin-evm_version_history.json
- CSV:   stablecoin-evm_commits.csv

Usage:
    python extract_stablecoin_evm.py
